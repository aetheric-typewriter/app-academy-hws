...
def below_average_years
  ...
  Movie.select('yr').where('count(score < 5)').group('yr').order('desc')
end
...
def alphabetized_actors
  ...
  Actor.select('name').order('desc').limit(10)
end
...
def pulp_fiction_actors
  ...
  Actor.select('actor.id, actor.name').joins(:castings).joins(:movies).where('movie.name = ?', 'Pulp Fiction')
end
...
def uma_movies
  ...
  Movies.select('movie.id, movie.title, movie.yr').joins(:castings).joins(:actors).where('actor.name = ?', 'Uma Thurman').order('desc')
end
